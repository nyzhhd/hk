...
int main2223()
{
	Net_config cfg = { 0.8, 0.5, "D://adavance//yolo_free_huge_crowdhuman_192x320.onnx", "person","D://adavance//coco.txt" };
	//FreeYOLO net(cfg);

	cv::dnn::Net net = cv::dnn::readNetFromONNX("D://adavance//yolo_free_huge_crowdhuman_192x320.onnx");  // 加载训练好的识别模型
	cv::Mat image = cv::imread("C://Users//wcz13//Pictures//me//xjpic.jpg");  // 读取图片
	cv::Mat blob = cv::dnn::blobFromImage(image);  // 由图片加载数据 这里还可以进行缩放、归一化等预处理
	net.setInput(blob);  // 设置模型输入
	cv::Mat predict = net.forward(); // 推理出结果
	
	string imgpath = "C://Users//wcz13//Pictures//me//xjpic.jpg";
